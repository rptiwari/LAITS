...
class PopupCanvasMenu extends JPopupMenu implements ActionListener, PopupMenuListener {
  ...
  public PopupCanvasMenu(final Selectable sx, final GraphCanvas cx, final Graph gx, Task t, JMenuItem run, JMenuItem takeQuiz, JFrame jf) {
    ...
    JMenuItem titleMenu = new JMenuItem(v.label);
    ...
    for (int i = 0; i < vertexType.length; i++) {
      ...
      if (((Vertex)sx).type.equals(vertexType[i])) vertexTypeList.setSelected(true);
    ...
    if((((Vertex)sx).equation == null) || (((Vertex)sx).equation.getIsCorrect() == false) || (((Vertex)sx).getAlreadyRun() == false) || graphcanvas.getModelChanged() == true || ((Vertex)sx).getGraphOpen() == true)
    {
  ...
  public void actionPerformed(ActionEvent e) {    
    if (e.getActionCommand().equals("Equation") && selected instanceof Vertex) {
      ...
      if(((Vertex)selected).type.contains("none"))
      {
      ...
      else
      {
        ...
        if (((Vertex)selected).type.equalsIgnoreCase("none"))
        {
            MessageDialog.showMessageDialog(null, true, "Please select a type for this node!", graph);
        }
        else if((((Vertex)selected).equation == null) || (((Vertex)selected).equation.getIsCorrect() == false) || (((Vertex)selected).getAlreadyRun() == false) || graphcanvas.getModelChanged() == true)        {
        ...
        else
        {
            ...
            logs.concatOut(Logger.ACTIVITY, "PopupCanvasMenu.actionPerformed.6", ((Vertex)selected).label);
      ...
      v.type = e.getActionCommand();
      graphcanvas.setModelChanged(true);
      logs.concatOut(Logger.ACTIVITY, "PopupCanvasMenu.actionPerformed.7", v.label + "-" + v.type);
      if(v.type.equalsIgnoreCase("none"))
      {
