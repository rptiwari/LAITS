...
import amt.gui.CongratulationsDialog;
...
public class GraphCanvas extends JPanel implements FocusListener, ActionListener, KeyListener, MouseListener, MouseMotionListener, ComponentListener, Scrollable {
  ...
  /**
   * This method initializes a button to show the short description of the problem
   */
      ...
      public void actionPerformed(java.awt.event.ActionEvent evt) {
        shortDescriptionActionPerformed(evt, gc);
            ...
            private void shortDescriptionActionPerformed(ActionEvent evt, GraphCanvas gc) {
                ...
                else paintDescriptionText = false;
            }
    });
  }
  ...
  public void initIcons() {
    ...
    if (Main.professorVersion != Main.VERSION2 && Main.professorVersion != Main.DEMO_VERSION2) {
      URL graphURL = Main.class.getResource("images/graph.png");
      graphImage = (toolkit.createImage(graphURL));
      URL graphDisabledURL = Main.class.getResource("images/graphDisabled.png");
      graphDisabledImage = toolkit.createImage(graphDisabledURL);
      URL edgeURL = Main.class.getResource("images/arrow.png");
      edgeImage = (toolkit.createImage(edgeURL));
      URL equationURL = Main.class.getResource("images/math.png");
      equationImage = (toolkit.createImage(equationURL));
      URL equationDisabledURL = Main.class.getResource("images/mathDisabled.png");
  ...
  /**
   * this method returns whether the user passed the task and moves on to the last level
  ...
  /**
   * This method returns a list of each problem that has been completed in a level.
   * If the size of this list equals problemList.get(currentLevel).length than we
   * know that all problems within a task has been completed.
   *  - Used for VERSION2 
   * @return
   */
  ...
  public final void paintComponent(Graphics g) {
    ...
    else runButton.setForeground(Color.BLACK);

    if(takeQuiz.getForeground() == Color.GRAY)
    {
        takeQuizButton.setForeground(Color.GRAY);
    }
    else takeQuizButton.setForeground(Color.BLACK);
...
/*commented out by zpwn: we do not need in New Node feature      
            int[] indices = suffledIndexes(listOfVertexes);
            int height = this.getParent().getHeight();
            this.setFont(this.normal);
            for (int i = 0; i < listOfVertexes.size(); i++) {
                if(Math.floor(i/6) > 0)
                {
                    this.newVertex(100 + i%6 * 125, height - (int)(v.paintNoneHeight * 2 * (Math.floor(i/6) + 1)), listOfVertexes.get(indices[i]));
                }
                else this.newVertex(100 + i * 125, height - v.paintNoneHeight * 2, listOfVertexes.get(indices[i]));
            }
...
*/
  ...
  public boolean canRun()
  {
      ...
      for (int j = 0; j < n; j++) {
        ...
        v = (Vertex) a[j];
        if(v.getEditorOpen() == false && !v.type.equalsIgnoreCase("none") && !v.type.equalsIgnoreCase("constant") /*&& v.equation != null*/) {
           // do nothing
    ...
    if(noneCount > 0/*noneCount == graph.getVertexes().size()*/) {
      runnable = false;
    }
  ...
  public void getTaskInformation(String title, String description) {
    ...
    else componentWidth = (int)frame.getToolkit().getDefaultToolkit().getScreenSize().getWidth();
  ...
  /**
   * Method to implement actions if the mouse is dragged.
   * Notice that now it does not do anything
   *
   * @param e MouseEvent
   */
  @Override
  public void mouseDragged(MouseEvent e) {
    ...
    if(x < 0) {
      x = 0;
        ...
        if (mouseDraggedDescription(x, y, e)) return;
        if (mouseDraggedAvatar(x, y, e)) return;
        if (mouseDraggedVertex(x, y, e) && !mouseDraggedDescription(x, y, e)) return;
        if (mouseDraggedEdge(x, y, e) && !mouseDraggedDescription(x, y, e)) return;
  }
  ...
  /**
   * Method to handle the possibility that we are dragging allEdges Vertex or allEdges Vertex label.
   * @param x Mouse x position
   * @param y Mouse y position
   * @param e Mouse event
   */
  ...
  /**
   * Method to handle the possibility that we are dragging an Edge or an Edge label.
   *
   * @param x Mouse x position
   * @param y Mouse y position
   * @param e Mouse event
   ...
   * @param x Mouse x position
   * @param y Mouse y position
   * @return If exist the hitted curved edge, otherwise keep checking with hitEdge
   */
  ...
  */
  ...
  /**
   * Method to verify if we are hiting the mouse over the label of allEdges vertex.
   * @param x Mouse x position
   * @param y Mouse y position
  ...
  /**
   * Method to alter the size (by d) of all items of the same type as the selected item.
   */
  ...
  public void keyPressed(KeyEvent e) {
    ...
    public void keyTyped(KeyEvent e)
    {
        ...
        { // newline
            graph.unselect();
            // System.out.println("repaint by keyTyped1");
            repaint(0);
            return;
        }
        ...
        if (graph.selected != null)
        {
            ...
            // System.out.println("repaint by keyTyped2");
            repaint(0);
        }*/
    }
    ...
    /**
     * If changeShape is set by the popup menu,
     * reset the control point with current mouse position.
     * @param e Mouse event
     */
    @Override
   ...
   /**
    ...
    /**
     * Method that review if there is an edge between Vertex allEdges and Vertex b
...
//    private boolean existEdgeBetween(Vertex allEdges, Vertex b){
  ...
  /**
   * Method to create allEdges new Vertex, in an x,y position and with allEdges label name.
   * This methods is only used to do the initialization.
   *
