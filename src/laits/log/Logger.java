...
/**
 * Class Logger.
 * Log the student activity and the system bugs into the database
 *
...
public class Logger {
  ...
  static {
    ...
    ACTIVITYMSG.put("Main.setProfessorVersion.1", "Running student version");
    ACTIVITYMSG.put("Main.setProfessorVersion.2", "Running professor version");
    ACTIVITYMSG.put("Main.setProfessorVersion.3", "Running version 1 demo");
    ACTIVITYMSG.put("Main.setProfessorVersion.4", "Running version 2");
    ACTIVITYMSG.put("Main.setProfessorVersion.5", "Running version 2 demo");
    ...
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.1", "Node editor opens at the Description Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.2", "Node editor opens at the Inputs Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.3", "Node editor opens at the Calculations Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.4", "Node editor opens at the Graphs Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.5", "Student working in the Description Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.6", "Student working in the Inputs Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.7", "Student working in the Calculations Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.8", "Student working in the Graphs Tab");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.9", "Student chose to close the Node editor");
    ACTIVITYMSG.put("TabbedGUI.TabbedGUI.10", "Student opens the Node editor for ");
  ...
  private static final String FILEDEBUG = "logDB.txt";
  private static final String FILEACTIVITY = "activityDB.txt";
  ...
  private ActSocketServer actSocketServer = null;
  ...
  private Logger() {
    ...
    if (Main.professorVersion != Main.DEMO && Main.professorVersion != Main.DEMO_VERSION2) {
      try {
        ...
        actSocketServer = ActSocketServer.getActSocketServer();
  ...
  /**
   * This method adds a log and should only be used for dynamic logs like when the student
   * changes to a new task
...
//  }
  /**
   * Log a message in the corresponding file. There is one file for DEBUG messages
   * and other for ACTIVITY messages.
   *
...
//          LogWriter.write(time + " + " + LOGMSG.get(key) + msg);
  ...
  /**
   * Close logging.
   * THIS METHOD MUST BE CALLED WHEN student CLOSE THE APPLICATION!!!!
   * CLOSE THE FILES LOG AND ACTIVITY WRITERS - AND OPEN THE FILES FOR READING
   */
  public void close(Frame p, ClosingDialog c) throws CommException {
    if (Main.professorVersion != Main.DEMO && Main.professorVersion != Main.DEMO_VERSION2) {
      ...
      this.actSocketServer.close();
    }
  }

  /**
   * This method reads the number of lines in the log file and puts each line into the linked list
   *
  ...
  /**
   * This method reads the number of lines in the activity file and puts each line into the linked list
   *
   * @param file is the name of the file
   * @return is the number of lines in the file
   */
