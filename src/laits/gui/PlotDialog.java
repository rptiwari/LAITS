package laits.gui;

import laits.data.Task;
import laits.graph.Graph;
import laits.graph.Vertex;
import java.awt.Component;
import java.awt.event.WindowEvent;
import java.awt.event.WindowListener;
import javax.swing.JPanel;

/**
 * Class to draw graphs (plots)
 *
 * @author javiergs y helenchavez
 * @version 20090719
 *
 */

public class PlotDialog extends javax.swing.JDialog implements WindowListener {

  JPanel grafica;
  Vertex node;
  Graph graph;
  //the width and height of the panel
  int width, height;

  public PlotDialog(/*java.awt.Frame parent,*/ boolean modal, Vertex node, Graph g, Task t) {
    //super(parent, modal);
    initComponents();
    this.node = node;
    this.graph = g;
    this.setTitle(this.node.label + " Graph");
    this.setAlwaysOnTop(false);
    //this.setLocationRelativeTo(parent);
    //height = (int)this.size().getHeight();
    //width = (int)this.size().getWidth();
    this.pack();

    if (grafica != null) {
      graphPanel.remove(grafica);
    }
    //grafica = new laits.plot.PlotPanel (this.node, Integer.parseInt(x0TextField.getText()), Integer.parseInt(xFTextField.getText()), (String)relationComboBox.getSelectedItem(), graph);
    grafica = new laits.plot.PlotPanel (this.node, t.getStartTime(), t.getEndTime(), graph, t.getUnitTime());
    graphPanel.add(grafica);
    Panel.add(graphPanel);
    graph.getPlots().add(this);
    //Panel.add(plotButton);
    //Panel.add(cancelButton);
    this.add(Panel);
    this.setVisible(true);
    this.addWindowListener(this);
  }

  public PlotDialog(java.awt.Frame frame,Component parent, boolean modal, Vertex node, Graph g, Task t)
  {
    super(frame, modal);
    System.out.println("OPEN!");
    initComponents();
    this.node = node;
    this.graph = g;
    this.setTitle(this.node.label + " Graph");
    //this.setLocationRelativeTo(parent);
    //width = (int)this.size().getWidth();
    //height = (int)this.size().getHeight();
    this.setAlwaysOnTop(false);
    this.pack();

    if (grafica != null) {
      graphPanel.remove(grafica);
    }
    //grafica = new laits.plot.PlotPanel (this.node, Integer.parseInt(x0TextField.getText()), Integer.parseInt(xFTextField.getText()), (String)relationComboBox.getSelectedItem(), graph);
    grafica = new laits.plot.PlotPanel (this.node, t.getStartTime(), t.getEndTime(), graph, t.getUnitTime());
    graphPanel.add(grafica);
    Panel.add(graphPanel);
    graph.getPlots().add(this);
    //Panel.add(plotButton);
    //Panel.add(cancelButton);
    this.add(Panel);
    this.setVisible(true);
    this.setLocationRelativeTo(parent);
    this.addWindowListener(this);
  }

  /**
   * Returns the vertex that the graph is for
   * @return the vertex
   */
  public Vertex getVertex()
  {
      return node;
  }
  
  /** This method is called from within the constructor to
   * initialize the form.
   * WARNING: Do NOT modify this code. The content of this method is
   * always regenerated by the Form Editor.
   */
   @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        Panel = new javax.swing.JPanel();
        graphPanel = new javax.swing.JPanel();

        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setResizable(false);

        graphPanel.setMinimumSize(new java.awt.Dimension(450, 216));
        graphPanel.setPreferredSize(new java.awt.Dimension(450, 216));
        graphPanel.setRequestFocusEnabled(false);
        graphPanel.setLayout(new java.awt.GridLayout(1, 0));

        javax.swing.GroupLayout PanelLayout = new javax.swing.GroupLayout(Panel);
        Panel.setLayout(PanelLayout);
        PanelLayout.setHorizontalGroup(
            PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(graphPanel, javax.swing.GroupLayout.DEFAULT_SIZE, 480, Short.MAX_VALUE)
        );
        PanelLayout.setVerticalGroup(
            PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(graphPanel, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 263, Short.MAX_VALUE)
        );

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(Panel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

       
    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JPanel Panel;
    private javax.swing.JPanel graphPanel;
    // End of variables declaration//GEN-END:variables

    public void windowOpened(WindowEvent e){}

    public void windowClosing(WindowEvent e){}

    public void windowClosed(WindowEvent e)
    {
        node.setGraphOpen(false);
        graph.getPlots().remove(this);
    }

    public void windowIconified(WindowEvent e){}

    public void windowDeiconified(WindowEvent e){}

    public void windowActivated(WindowEvent e){}

    public void windowDeactivated(WindowEvent e){}
}
