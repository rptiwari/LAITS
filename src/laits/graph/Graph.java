 ...
 * @version 20100116
 */
public class Graph extends Selectable implements Serializable{
   ...
   */
   ...
   // Updated by Ram - 1 Feb
        
        ListIterator<Edge> itr = v.inedges.listIterator();
        while(itr.hasNext()){
            Edge e = itr.next();
            edges.remove(e);
            e.start.outedges.remove(e);        
        }

        itr = v.outedges.listIterator();
        while(itr.hasNext()){
            Edge e = itr.next();
            edges.remove(e);
            e.end.inedges.remove(e);            
            e.end.equation = null;
            e.end.stockEquation = null;
        }
        vertexes.remove(v);        
    }
   ...
   */
    public void load(File f) throws DocumentException {
        ...
        for (Element e : vertexNode) {
            ...
            Element nodeName = e.element("nodeName");
            Element correctDescription = e.element("correctDescription");
            ...
            v.type = type.getText();
            // Restoring Node values from File - Ram
            v.nodeName = nodeName.getText();
            v.correctDescription = correctDescription.getText();
            v.selectedDescription = v.correctDescription;
            v.setDescriptionButtonStatus(1);
